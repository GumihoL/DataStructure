...
public class BST<Key extends Comparable<Key>> {
    ...
    public double computeAveDepth(){
        return (double)sumDepth(root)/size(root);
    }

    /**
     * @param x subtree root.
     * @return return an internal path length of a subtree rooted at x.
     */
    private int sumDepth(Node x){
        if (x == null) return 0;
        return size(x) - 1 + sumDepth(x.left) + sumDepth(x.right);
    }
}
